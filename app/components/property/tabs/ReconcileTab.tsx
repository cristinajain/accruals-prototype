    ...
    <>
      ...
      <div className="sp-accrual-list sp-mb-24">
        {ACTUALS_DATA.filter(a => !a.spread).map(act => {
          ...
          return (
            <div key={act.id} className={`sp-accrual-row${isExp ? " sp-accrual-row--expanded" : ""}`} style={{ opacity: rS === "reconciled" ? 0.7 : 1 }}>
              <div onClick={() => setExpandedActual(isExp ? null : act.id)} className="sp-accrual-row__top" style={{ gridTemplateColumns: chatOpen ? "1fr 80px 80px 100px" : "1fr 100px 100px 100px 150px" }}>
                <div><div className="sp-flex-center sp-gap-5 sp-mb-2"><span className="sp-text-md-semibold">{act.vendor}</span><Badge color={sCfg.c}>{sCfg.l}</Badge>{rS === "reconciled" && <Badge color="green">Posted</Badge>}</div><div className="sp-text-sm-muted">{act.glCode} · {act.invoiceNum}</div></div>
      ...
      <div className="sp-accrual-list sp-mb-20">
        {ACTUALS_DATA.filter(a => a.spread).map(act => {
          ...
          return (
            <div key={act.id} className={`sp-accrual-row${isExp ? " sp-accrual-row--expanded" : ""}`} style={{ opacity: rS === "reconciled" ? 0.7 : 1 }}>
              <div onClick={() => setExpandedActual(isExp ? null : act.id)} className="sp-accrual-row__top" style={{ gridTemplateColumns: chatOpen ? "1fr 80px 80px 100px" : "1fr 100px 100px 100px 150px" }}>
                <div><div className="sp-flex-center sp-gap-5 sp-mb-2"><span className="sp-text-md-semibold">{act.vendor}</span><Badge>Multi-Period</Badge><Badge>{mLbl}</Badge>{rS === "reconciled" && <Badge color="green">Posted</Badge>}</div><div className="sp-text-sm-muted">{act.glCode} · {sp.startMonth} → {sp.endMonth}</div></div>
